...
bool DSMNode::is_in_range(char * addr) {
    return (intptr_t)addr >= (intptr_t)this->base && (intptr_t)addr < ((intptr_t)this->base) + PAGE_SIZE * this->page_info.size();
}
...
void DSMNode::sync() {
    for (int i = 0; i < this->page_info.size(); i++) {
        if (OWNERSHIP(this->page_info[i])) continue;
        mprotect(relative_page_id_to_addr(i), PAGE_SIZE, PROT_NONE);
    }
}

DSMNode::DSMNode(NodeAddr m_addr, void * _base, size_t _len, bool is_master, int swapfd) {
    ...
    serv->bind(RPC_JOIN, [this]() -> vector<NodeAddr> {
